...
```mermaid
graph TD
    Client[Frontend Client SPA] -->|REST API / JWT| API[FastAPI Web Server]
    
    subgraph Services[Core Engine Services]
        API --> Auth[JWT Auth Guard]
        API --> Gap[Skill Gap Analyzer]
        API --> Rec[Recommendation Matcher]
        API --> Path[Roadmap Generator]
        API --> Quiz[Adaptive Assessment]
    end

    subgraph Storage[Persistence Layer]
        Gap --> DB[(Database: SQLite / PostgreSQL)]
        Rec --> DB
        Path --> DB
        Quiz --> DB
    end
...
```mermaid
graph LR
    A[Onboarding] --> B[Skill Gap Analysis]
    B --> C[Priority Queue]
    C --> D[Prerequisite DAG]
    D --> E[Recommendation Engine]
    E --> F[Dashboard Timeline]
    F --> G[Adaptive Quiz]
    G -->|Recalibrate| B
```
